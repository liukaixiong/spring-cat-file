...
import com.cat.file.message.storage.BlockDumper;
import com.cat.file.message.storage.exception.BlockQueueFullException;
...
public class DefaultBlockDumper extends ContainerHolder implements BlockDumper {
    ...
    public void dump(Block block) throws IOException {
        ...
        if (!success) {
            ...
            if ((++m_failCount % 100) == 0) {
                LogUtils.logError(new BlockQueueFullException("Error when adding block to queue, fails: " + m_failCount));
    ...
    public void initialize(int hour) {
        int threads = storageProperties.getMessageDumpThreads();
